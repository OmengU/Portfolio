...
const showDialog = (dialogkey) => {
    ...
    <div>
    ${dialogs[dialogkey].names.map(name => `<p>${name}</p>`
    ).join('')}
    </div>
    <div>
    ${dialogs[dialogkey].points.map(points => `<p>${`<span class="circleFilledIcon icon circleIcon"></span>`.repeat(points).concat(`<span class="circleOutlineIcon icon circleIcon"></span>`.repeat(6-points))}</p>`
    ).join('')}
    </div>
</div>
</div>`
